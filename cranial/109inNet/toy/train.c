...
int main(int argc, char* argv[]){
  ...
  const unsigned int num_hidden = 5; //, num_hidden2 = 7;

  struct fann_train_data *data = fann_read_train_from_file("toy.train");
 ...
 fann_set_learning_rate(ann, .003);
  ...
  struct fann_train_data *data2 = fann_read_train_from_file("toy.test");
  ...
  for(i=0; i< fann_length_train_data(data2); i++, inputData+= 109, outputData++)
  {
    ...
    for(j=0; j < 109; j++)
      printf("input %d is %f \n ", j, inputData[j]);
    printf("expected output %d is %f \n ", 0, outputData[0]);
    calc_out = fann_run(ann, inputData);
    printf("calculated output %d is %f \n ", 0, calc_out[0]);
    if (outputData[0] > 0){
      total_male++;
      if(calc_out[0] < .5){
        wrong_male++;
