...
int main(int argc, char* argv[]){
  const unsigned int num_input = 109;
  const unsigned int num_output = 1;
  const unsigned int num_layers = 3;
  const unsigned int num_hidden = 4; //, num_hidden2 = 7;

  struct fann_train_data *data = fann_read_train_from_file("AvgFilling.train");
  struct fann *ann = fann_create_standard(num_layers, num_input, num_hidden, num_output);
  int max_epochs, epochs_between_reports = 100;
  ...
  fann_set_learning_momentum(ann, .5);
  ...
  struct fann_train_data *data2 = fann_read_train_from_file("AvgFilling.test");
  ...
  for(i=0; i< fann_length_train_data(data2); i++, inputData+= 109, outputData++)
  {
