...
int main(int argc, char* argv[]){
  ...
  struct fann_train_data *data = fann_read_train_from_file("allUnequal1.train");
  ...
  struct fann_train_data *data2 = fann_read_train_from_file("allUnequal1.test");
  ...
  for(i=0; i< fann_length_train_data(data2); i++, inputData++, outputData++)
  {
    if(outputData[0] > 0)
      total_male++;
    else
